...
function sendInputs() {
    ...
    let result = arrayQueryPrimaryDiv.map(x => {
        ...
        inputsValues.inputOptions = inputsValues.arrayInputOptions.map(y =>{let valueInput = y.value; return valueInput});
        inputsValues.arrayOptionValue = Array.from(x.querySelectorAll('.valueInput'));
        inputsValues.optionValue = inputsValues.arrayOptionValue.map(y =>{let valueInput = y.value; return valueInput});
        return inputsValues
    });
    console.log(result)
}